...
Finally, a list of provided IMS startup commands provided by IBM used within KBC are explained in \textcite{long2000ims}:
\begin{itemize}
	\item Cold start: this is done the first time the system is started. During this process the message queues, dynamic logs and restart data sets are initialized
	\item Emergency restart: when a failure occurs IMS will be restarted according to the log active at the time of the failure. This process will black-out the database changes of incomplete message processing programs (MPPs)
		and batch message processing programs (BMPs). After this step the input messages are re-enqueued and the MPPs are restarted as well as retransmitting the output messages. BMPs must be resubmitted however, this is 
		to prevent missing or inconsisten output.
	\item Warm start (referred to as "normal restart" in \textcite{long2000ims}): to preserve the message queues, a normal restart is done from a previous normal IMS termination. 
\end{itemize}
