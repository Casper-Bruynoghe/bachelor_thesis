...
\section{Inleiding}
\label{sec:inleiding}
In dit onderzoek wordt er specifiek gekeken naar het IMS subsysteem van de mobile en touch platformen binnen KBC. De scope van dit onderzoek (en proof-of-concept) beperkt zich tot de test omgeving, de ‘testplex’, 
waar alles uitgetest wordt vooraleer het uitgerold wordt naar de andere IMS systemen. Bij het opstarten van een IMS-subsysteem zijn er drie soorten die door IBM worden aangeboden: 
\emph{Warm start}, \emph{Cold start} en \emph{Emergency restart}. Binnen KBC heeft het Automation-team (subteam binnen Mainframe infrastructuur) uniforme start- en stopprocedures voorzien voor een groot aantal situaties. 
Er zijn ook een aantal gespecialiseerde procedures ontwikkeld die niet in de IBM-handleidingen te vinden zijn. Er moet dus een onderscheid worden gemaakt tussen wat IBM heeft voorzien – de drie hierboven beschreven 
soorten opstartprocedures – en wat via Automation in KBC is ingebouwd. Elke procedure is gebaseerd op het opstarten van het IMS-(sub)systeem, maar er worden ook aanvullende controles en commando's uitgevoerd, 
zoals het opstarten van regions, het opstarten van IMS-verbindingen, enz.\\
...
%   ...'')
\textcite{alex2012transaction} definieert een transactie als een interactie met een informatiesysteem. Enkele voorbeelden hiervan zijn gegeven door \textcite{alex2012transaction}: het bijwerken van een winkelboekhouding, 
het goedkeuren van een creditcardtransactie of het verwijderen van een artikel uit de winkelvoorraad tijdens het afrekenen. Ook wordt het belang van deze systemen uiteengezet. 
Als een transactie niet wordt verwerkt, kan dit ertoe leiden dat een klant het dubbele van het verschuldigde bedrag in rekening wordt gebracht. \\
...
Binnen KBC wordt Total Object Manager (TOM) gebruikt om objecten te starten en stoppen. TOM beheert wanneer objecten starten of stoppen door veranderingen in de status, definities, schema's en afhankelijkheden van
objecten te evalueren en vervolgens de werkelijke systeemstatus van het object te vergelijken met de gewenste status \autocite{BMCTotalObjectManager}.\\

Ten slotte wordt in \textcite{long2000ims} een lijst gegeven van door IBM verstrekte IMS-opstartcommando's die binnen KBC worden gebruikt:
\begin{itemize}
	\item Cold start: dit gebeurt wanneer het systeem voor het eerst wordt opgestart. Tijdens dit proces worden de message queues, dynamische logboeken en herstart datasets geïnitialiseerd \autocite{long2000ims}. 
		\textcite{IMSOperationsAndAutomation} geeft nog twee andere situaties waarbij een IMS systeem een cold start moet ondergaan: nadat de nucleus wordt veranderd tijdens een system definition en na een reallocatie van
		de shared-message-queue structures als deze messages bevatte.
	\item Warm start (in \textcite{long2000ims} aangeduid als “normal restart”): om de message queues te behouden, wordt een normale herstart uitgevoerd vanaf een eerdere normale IMS-beëindiging. 
		Voordat IMS kan gestart worden moet er bepaald worden of IMS de MSDBs (dit zijn de hoofd opslag databases \autocite{long2000ims}) moeten herladen worden, specifieke systeem data sets moeten 
		worden geformat en/of specifieke security functies moeten worden geactiveerd \autocite{IMSOperationsAndAutomation}.
	\item Emergency restart: wanneer er een storing optreedt, wordt IMS opnieuw opgestart volgens het logboek dat actief was op het moment van de storing. Dit proces zorgt ervoor dat de databasewijzigingen 
		van onvolledige message processing programs (MPPs) en batch message processing programs (BMPs) worden geblokkeerd. Na deze stap worden de invoerberichten opnieuw in de wachtrij geplaatst en worden 
		de MPP's opnieuw opgestart en worden de uitvoerberichten opnieuw verzonden. BMPs moeten echter opnieuw worden ingediend om ontbrekende of inconsistente uitvoer te voorkomen. 
		\autocite{long2000ims}\\

		Voordat een emergency restart kan worden uitgevoerd, moet er gecontroleerd worden of de systeem datasets opnieuw gealloceerd en geformatteerd moeten worden en/of IMS zijn resources heeft opgekuist \autocite{IMSOperationsAndAutomation}.
		
\end{itemize}
Het verschil tussen MPPs en BMPs wordt uitgelegd in \textcite{mcgee1977information}:
\begin{itemize}
	\item MPPs worden gebruikt om applicaties uit te voeren wanneer er een message binnenkomt. Deze worden voornamelijk gebruikt voor online transacties.
	\item BMPs daarentegen worden gebruikt voor door de operator geplande programma's die toegang vereisen tot de online databases of de message queues.
\end{itemize}
...
\begin{itemize}
	\item \textcolor{red}{Welke onderhoudsstrategie op lange termijn kan voorkomen dat performantieverlies zich in de toekomst opnieuw voordoet?}
...
van de vereisten grondig genoeg wordt uitgevoerd, zodat er niets over het hoofd wordt gezien. Hierdoor wordt het risico dat symptomen worden aangepakt in plaats van de onderliggende oorzaken verminderd.
...
De proof of concept moet minimum voldoen aan de vereisten die gedefinieerd werden in fase 2 en moet een significante vermindering hebben in elapstijd bij de AUTOSTART/AUTOSTOP procedures om aanvaard te worden.
