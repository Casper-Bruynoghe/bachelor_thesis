...
As this research is about improving a system, it is imperative to know how to approach such a task. An important point raised in \textcite{amra2017modernizing} explains that iterative and incremental processes, also
called the agile approach, are critical in tackling these types of projects. It prevents teams from being overwhelmed or frustrated when trying to completely overhaul their entire system in one go.
Testing is another point highlighted in this book, it is the only way to ensure the system keeps working after changing parts of it. Two types of tests are core pillars of this workflow:
\begin{itemize}
	\item Regression testing: these test ensure that the all functionalities work as expected when changing only a single part of it.
	\item Automatic unit testing: these tests allow the development team to test small parts of the application, this in turn requires a deeper understanding of the codebase. These can be set up to run automatically 
		after each compilation, therefore they are required to be able to run fast to not hinder development.
\end{itemize}\\
